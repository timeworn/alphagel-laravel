...
class ProductsModel extends Model
{
    ...
    public function getMostSelledProducts($order_postion = "asc")
    {
        $products = DB::table('products')
                ->select(DB::raw('products.*, products.id as p_id, products_translations.*'))
                ->where('hidden', '=', 0)
                ->where('locale', '=', app()->getLocale())
                ->join('products_translations', 'products_translations.for_id', '=', 'products.id')
                ->orderBy('order_position', $order_postion)
                ->limit(8)
                ->get();
        return $products;
    }
