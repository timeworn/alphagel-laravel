            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ __('admin_pages.ord_'.$order->status) }}</td>
                        <td>{{ $order->phone }}</td>
                        <td>
                            <select class="selectpicker change-ord-status" data-ord-id="{{$order->orderId}}" data-style="btn-secondary"> 
                                <option {{ $order->status == 0 ? 'selected="selected"' : '' }} value="0">{{__('admin_pages.ord_0')}}</option>
                                <option {{ $order->status == 1 ? 'selected="selected"' : '' }} value="1">{{__('admin_pages.ord_1')}}</option>
                                <option {{ $order->status == 2 ? 'selected="selected"' : '' }} value="2">{{__('admin_pages.ord_2')}}</option>
                                <option {{ $order->status == 3 ? 'selected="selected"' : '' }} value="2">{{__('admin_pages.ord_3')}}</option>
                                <option {{ $order->status == 4 ? 'selected="selected"' : '' }} value="2">{{__('admin_pages.ord_4')}}</option>
                                <option {{ $order->status == 5 ? 'selected="selected"' : '' }} value="2">{{__('admin_pages.ord_5')}}</option>
                                <option {{ $order->status == 6 ? 'selected="selected"' : '' }} value="2">{{__('admin_pages.ord_6')}}</option>
                            </select>
                        </td>
                        <td class="text-right">
                    ...
                    <tr class="tr-more" data-tr="{{ $order->order_id }}">
                        <td colspan="6">
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    @php
                                    $producta = $controller->getProductInfo($order->products);
                                    @endphp
                                    <div class="product">
                                        <a>
